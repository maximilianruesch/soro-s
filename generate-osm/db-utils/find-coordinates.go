...
func FindNewNode(
	osm *OSMUtil.Osm,
	nearestAnchor *OSMUtil.Node,
	secondNearestAnchor *OSMUtil.Node,
	nearestAnchorDistance float64,
	secondNearestAnchorDistance float64,
) (*OSMUtil.Node, error) {
	if nearestAnchorDistance == 0.0 {
		return nearestAnchor, nil
	}

	var node *OSMUtil.Node

	up1, upDist1, down1, downDist1, err1 := findNodes(osm, nearestAnchor, nearestAnchorDistance)
	up2, upDist2, down2, downDist2, err2 := findNodes(osm, secondNearestAnchor, secondNearestAnchorDistance)
	...
	if up1 == up2 {
		node, _ = OSMUtil.GetNodeById(osm, up1)
	} else if up1 == down2 {
		node, _ = OSMUtil.GetNodeById(osm, up1)
	} else if down1 == up2 {
		node, _ = OSMUtil.GetNodeById(osm, down1)
	} else if down1 == down2 {
		node, _ = OSMUtil.GetNodeById(osm, down1)
	} else {
		node = getClosestMatch(osm, up1, up2, down1, down2, upDist1, upDist2, downDist1, downDist2)
	}
	return node, nil
}

func getClosestMatch(osm *OSMUtil.Osm, up1, up2, down1, down2 string, upDist1, upDist2, downDist1, downDist2 float64) *OSMUtil.Node {
	upNode1, _ := OSMUtil.GetNodeById(osm, up1)
	upNode2, _ := OSMUtil.GetNodeById(osm, up2)
	downNode1, _ := OSMUtil.GetNodeById(osm, down1)
	downNode2, _ := OSMUtil.GetNodeById(osm, down2)
...
func findNodes(
	osm *OSMUtil.Osm,
	anchorNode *OSMUtil.Node,
	anchorDistance float64,
) (string, float64, string, float64, error) {
	startWay, err := OSMUtil.FindWayById(osm, anchorNode.Id)
	if err != nil {
		return "", 0, "", 0, err
	}
	if len(startWay) > 2 {
	...
	if len(startWay) == 1 {
		...
		index, err := OSMUtil.GetNodeIndexInWay(&runningWay, anchorNode.Id)
		if err != nil {
			panic(err)
		}
		upId, upDist = goDir(osm, runningWay, index, anchorDistance, true)      // going "up" first
		downId, downDist = goDir(osm, runningWay, index, anchorDistance, false) // then going "down"
		return upId, upDist, downId, downDist, nil
	}

	if startWay[0].Nd[0].Ref == anchorNode.Id && startWay[1].Nd[len(startWay[1].Nd)-1].Ref == anchorNode.Id {
		runningWay := startWay[1]
		upId, upDist = goDir(osm, runningWay, len(runningWay.Nd)-1, anchorDistance, true)
		runningWay = startWay[0]
		downId, downDist = goDir(osm, runningWay, 0, anchorDistance, false)
	} else if startWay[1].Nd[0].Ref == anchorNode.Id && startWay[0].Nd[len(startWay[0].Nd)-1].Ref == anchorNode.Id {
		runningWay := startWay[0]
		upId, upDist = goDir(osm, runningWay, len(runningWay.Nd)-1, anchorDistance, true)
		runningWay = startWay[1]
		downId, downDist = goDir(osm, runningWay, 0, anchorDistance, false)
...
func goDir(osm *OSMUtil.Osm, runningWay OSMUtil.Way, index int, dist float64, initialWayDirUp bool) (string, float64) {
	runningNode, _ := OSMUtil.GetNodeById(osm, runningWay.Nd[index].Ref)
	...
	for totalDist < dist {
		if (wayDirUp && index == 0) || (!wayDirUp && index == len(runningWay.Nd)-1) {
			runningWay, index, wayDirUp, err = findNextWay(osm, wayDirUp, index, runningNode, oldNode, runningWay)
			if err == endReached {
		...
		nextNode = getNextNode(osm, wayDirUp, index, runningWay)
...
func getNextNode(osm *OSMUtil.Osm, wayDirUp bool, index int, runningWay OSMUtil.Way) *OSMUtil.Node {
	if wayDirUp {
		nextNode, err := OSMUtil.GetNodeById(osm, runningWay.Nd[index-1].Ref)
		if err != nil {
			panic(err)
		}
		return nextNode
...
func findNextWay(osm *OSMUtil.Osm, wayDirUp bool, index int, runningNode *OSMUtil.Node, oldNode *OSMUtil.Node, runningWay OSMUtil.Way) (OSMUtil.Way, int, bool, error) {
	nextWays, err := OSMUtil.FindWayById(osm, runningNode.Id)
